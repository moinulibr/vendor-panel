...
if ($('.bookingrange').length) {
    ...
    const booking_range = (start, end) => {
        ...
        $(document).trigger('bookingRangeChanged', {
...
$(document).ready(function(){
	...
	$('#common_modal').on('shown.bs.modal', function () {
	    if ($('.datetime').length > 0) {
			$('.datetime').daterangepicker({
		        singleDatePicker: true,        // Only one date instead of a range
		        timePicker: true,              // Enable time selection
		        timePicker24Hour: false,        // 24-hour format
		        timePickerSeconds: false,
		        // startDate: moment(),       // Hide seconds picker
		        locale: {
		            format: 'YYYY-MM-DD hh:mm A' // Date + time format
		        }
		    });
		}
	});
	...
	// Sidebar Initiate
	init();
	$(document).on('mouseenter', '.sidebar-logo', function (e) {  
        e.stopPropagation();
    
        if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            $('body').addClass('expand-menu');
            $('.subdrop + ul').stop(true, true).slideDown();
        }
    });
    
    $(document).on('mouseleave', '.sidebar-logo', function (e) {
        e.stopPropagation();
    
        if ($('body').hasClass('mini-sidebar') && $('#toggle_btn').is(':visible')) {
            $('body').removeClass('expand-menu');
            $('.subdrop + ul').stop(true, true).slideUp();
        }
    });
...
$(document).ready(function(){
	...
	setTimeout(function() {
		var activeMenuItem = $('.sidebar-menu li.active');
		if (activeMenuItem.length > 0) {
			var sidebarInner = $('.sidebar-inner');
			var activeItemOffset = activeMenuItem.offset().top - sidebarInner.offset().top + sidebarInner.scrollTop();
			sidebarInner.slimScroll({
				scrollTo: activeItemOffset - 100 + 'px' // Adjust offset to center the item
			});
		}
	}, 100);
